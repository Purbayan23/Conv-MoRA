...
The authors' executable reference is not fully consistent with the paper-level specification. Inspection of `adaptation.py` shows that the `constrained_lora_down3` training branch does not invoke `replace_layers(...)` or `mark_only_lora_as_trainable(...)` within that branch. It should therefore not be treated as a reliable executable implementation of the paper's stated full-encoder ConvLoRA configuration. The separate `full_lora` branch has inconsistent model-object flow and is not used as the mapping here.
